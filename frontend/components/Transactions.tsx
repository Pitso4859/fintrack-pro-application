...
import { PlusCircle, Info, Calculator, ArrowRightLeft, RefreshCw, Tag, Loader2, AlertCircle, FileText, Inbox } from 'lucide-react';
...
const Transactions: React.FC<TransactionsProps> = ({ transactions, accounts, onAdd, currency }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Build payload using the fixed toSnakeCase function
    const newTxPayload = {
      date: formData.date,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              {sortedTransactions.length === 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
                    ...
                    <h4 className="text-lg font-bold text-slate-700 mb-2">No Transactions Yet</h4>
                    <p className="text-sm text-slate-500 max-w-md mb-6">
                      Your ledger is currently empty. Start by creating your first transaction using the form on the left.
                    </p>
                    <div className="flex flex-wrap gap-3 justify-center">
                      <div className="bg-blue-50 rounded-xl p-3 text-left">
                        <p className="text-[10px] font-black text-blue-600 uppercase tracking-wider mb-1">💡 Quick Tip</p>
                        <p className="text-xs text-blue-700">Create an INVOICE for sales revenue or an EXPENSE for business costs.</p>
                      </div>
                    </div>
                  </div>
              ) : (
            ...
            {/* Summary Footer - Only show if there are transactions */}
            {sortedTransactions.length > 0 && (
                <div className="p-6 border-t border-slate-100 bg-slate-50/50">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Transactions</p>
                      <p className="text-lg font-black text-slate-900">{sortedTransactions.length}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Value (Gross)</p>
                      <p className="text-lg font-black text-emerald-600">
                        {currency.symbol}
                        {sortedTransactions.reduce((sum, tx) => sum + tx.amount, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </p>
                    </div>
                  </div>
                </div>
